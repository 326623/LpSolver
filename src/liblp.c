...
int leaving_index(float* t, int* flag, int size) {
  ...
  for (i = 0; i < size; i++) {
    if (minpos_i < 0) {
      if ((flag[i] > 0) && (t[i] >= -EPS)) minpos_i = i;
    } else {
      if ((flag[i] > 0) && (t[i] >= -EPS) && (t[i] < t[minpos_i])) minpos_i = i;
    }
  }
  return minpos_i;
}
...
int compute_E(float* E, float* a, float* I, int size, int li) {
  ...
  for (i = 0; i < size; i++) a[i] = -a[i] / qth;
  a[li] = 1 / qth;
  for (i = 0; i < size; i++) E[(i * size) + li] = a[i];
  return 0;
}
void extract_column(float* M, float* v, int start_i, int stride, int size) {
  int i;
  for (i = 0; i < size; i++) v[i] = M[start_i + (i * stride)];
}
