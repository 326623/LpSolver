...
void display_array(const char* name, float* a, int m, int n) {
  ...
  printf("Array %s:\n", name);
  for (i = 0; i < m; i++) {
    for (j = 0; j < n; j++) printf("%f ", a[R2C(i, j, m)]);
    printf("\n");
  }
}
...
void display_int_array(const char* name, int* a, int m, int n) {
  ...
  printf("Int array %s:\n", name);
  for (i = 0; i < m; i++) {
    for (j = 0; j < n; j++) printf("%d ", a[R2C(i, j, m)]);
    printf("\n");
  }
}
/**
...
int read_array(FILE* file, float* a, int m, int n) {
  ...
  for (i = 0; i < m; i++)
    for (j = 0; j < n; j++) {
      fscanf(file, "%f",
             &a[R2C(i, j, m)]);  // Get the ith-element of the matrix from
    }                            // the command line, converting it
  // from text to float
  return 0;
}
// Release allocated memory
void free_array(void* a) { cudaFreeHost(a); }