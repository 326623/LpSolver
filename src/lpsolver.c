...
int main(int argc, char** argv) {
  ...
  switch (argc) {
    case 2:
      if (strcmp(argv[1], "-h") == 0 || strcmp(argv[1], "--help") == 0) {
        help();
      } else if ((sourcefile = fopen(argv[1], "r")) == NULL) {
  ...
  do {
    ...
    /* Timing */
    // y = cb*Binv
    cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, 1, m, m, 1, cb, m,
                Binv, m, 0, y, m);
    memcpy(&yb[1], y, m * sizeof(float));
    // e = [1 y]*[-c ; A]
    cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, 1, n, m + 1, 1, yb,
                n, D, n, 0, e, n);
    /* Timing */
    ...
    cblas_sgemv(CblasRowMajor, CblasNoTrans, m, m, 1, Binv, m, A_e, 1, 0, alpha,
                1);
